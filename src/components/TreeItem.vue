<template>
  <div class="tree-item" :style="{ 'padding-left': depth * 15 + 'px' }">
    ...
    <div
      ...
      :class="['item-label', 'file-label', { active: item.path === activeFilePath }]"
      role="link"
      tabindex="0"
      @keydown.enter="$emit('select-file', item.path)"
      :title="item.name"
    >
      <span class="item-name">{{ item.name }}</span>
    </div>
...
.item-children {
  /* 子项的容器，不需要额外样式，因为 TreeItem 自身会处理缩进 */
}
</style>