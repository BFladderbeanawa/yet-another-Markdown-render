...
export function slugify(text) {
  ...
  if (!s) { // 如果处理后 s 为空（例如输入是纯符号或空字符串），给一个默认值
    s = 'untitled-section';
  }
...
const originalLinkOpenRule = md.renderer.rules.link_open || function(tokens, idx, options, env, self) {
  return self.renderToken(tokens, idx, options);
};
...
export function parseMarkdown(markdown, basePath = '') {
  ...
  md.renderer.rules.image = (tokens, idx, options, env, self) => {
    ...
    if (src && currentBasePath && !/^(?:[a-z]+:|\/\/|\/|#|data:)/i.test(src)) {
      try {
        ...
        console.error("Error resolving relative image path in markdownParser:", e, "basePath:", currentBasePath, "src:", src);
        // 如果解析失败，保持原始 src
      }
      // 更新 token 中的 src 属性
  ...
  md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    if (hrefIndex >= 0) {
        ...
        // 只处理非外部、非锚点、非协议的链接
        if (href && currentBasePath && !/^(?:[a-z]+:|\/\/|\/|#|data:)/i.test(href)) {
            // 如果是 .md 文件，你可能想做特殊处理，比如转换为路由链接
            // 这里我们假设也像图片一样解析为相对于 public 的路径
            try {
                let baseForUrlConstructor = window.location.origin;
                if (currentBasePath.startsWith('/')) { baseForUrlConstructor += currentBasePath; }
                else { baseForUrlConstructor += '/' + currentBasePath; }
                if (!baseForUrlConstructor.endsWith('/')) { baseForUrlConstructor += '/'; }

                const resolvedUrl = new URL(href, baseForUrlConstructor);
                let finalHref = resolvedUrl.pathname;
                if (finalHref.startsWith('/')) { finalHref = finalHref.substring(1); }
                href = finalHref.replace(/\/+/g, '/');

            } catch (e) {
                console.error("Error resolving relative link path in markdownParser:", e, "basePath:", currentBasePath, "href:", href);
            }
...
export function splitMarkdownIntoBlocks(markdownText) {
  ...
  mditForSplit.use(mdAnchor, { // 确保用于分割的实例也有 anchor 以正确提取ID
      level: [2,3],
      slugify: s => slugify(s)
  });
  ...
  for (let i = 0; i < rawContentBlocks.length; i++) {
    ...
    const tokens = mditForSplit.parse(currentMarkdownBlock, {}); // 使用 mditForSplit 解析

    tokens.forEach((token) => { // 修正：移除 tokenIndex，直接用 token
      if (token.type === 'heading_open' && (token.tag === 'h2' || token.tag === 'h3')) {
        if (token.attrs) {
          const idAttr = token.attrs.find(attr => attr[0] === 'id');
          if (idAttr && idAttr[1]) {
            containedHeadingIds.push(idAttr[1]);
          }
        }
      }
    });
    ...
    processedBlocks.push({
      ...
      containedHeadingIds: containedHeadingIds,
    });
  }
  return processedBlocks;
}
// slugify 函数的 return `heading-${s}`; 最后一行有个错误，应该是 return `heading-${s}`;
// 我已经在上面 slugify 函数定义中修正了它。