...
async function initializeParser() {
  if (!mdWorkerIt) {
    try {
      ...
      mdWorkerIt = new MarkdownItModule.default({
        // 注意 .default
        html: true,
        linkify: true,
        typographer: true,
        highlight: function (str, lang) {
          ...
          return (
            '<pre class="hljs"><code>' +
            mdWorkerIt.utils.escapeHtml(str) +
            '</code></pre>'
          )
        }
      })
...
function workerSplitMarkdownIntoBlocks(markdownText) {
  ...
  const blocks = markdownText
    ...
    .reduce((acc, part, index, arr) => {
      if (index % 2 === 0) {
        if (part.trim()) {
          acc.push({
            id: `worker-block-${acc.length}-${Date.now()}`,
            markdown: part.trim()
          })
        }
      } else {
  ...
  if (blocks.length === 0 && markdownText.trim()) {
    return [
      { id: `worker-block-0-${Date.now()}`, markdown: markdownText.trim() }
    ]
  }
  return blocks.map((block, index) => ({
    ...block,
    id: `worker-block-${index}-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`
  }))
}
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    self.postMessage({
      ...
      originalId: markdownBlock ? markdownBlock.id : null
    })
  }
}
